...
fn style_accent() -> Style { Style::default().fg(C_ACCENT) }
fn style_dim()    -> Style { Style::default().fg(C_DIM) }
fn style_muted()  -> Style { Style::default().fg(C_GRAY) }
fn style_white()  -> Style { Style::default().fg(C_WHITE) }
fn style_bold(c: Color) -> Style { Style::default().fg(c).add_modifier(Modifier::BOLD) }
...
enum Action {
    Shell, Forward, Tunnel, Fwd, Copy, Sync, Ping, Proxy, Scan,
    Share, Watch, Punch, Route, Mount,
}

impl Action {
    const ALL: &'static [Action] = &[
        Action::Shell,   Action::Forward, Action::Tunnel, Action::Fwd,
        Action::Copy,    Action::Sync,    Action::Ping,   Action::Proxy,
        Action::Scan,
        // ── extra ──
        Action::Share,   Action::Watch,   Action::Punch,  Action::Route,
        Action::Mount,
    ];

    fn shortcut(self) -> char {
        match self {
            Action::Shell   => '1', Action::Forward => '2', Action::Tunnel  => '3',
            Action::Fwd     => '4', Action::Copy    => '5', Action::Sync    => '6',
            Action::Ping    => '7', Action::Proxy   => '8', Action::Scan    => '9',
            Action::Share   => '0', Action::Watch   => 'w', Action::Punch   => 'p',
            Action::Route   => 'r', Action::Mount   => 'm',
        }
    }

    fn icon(self) -> &'static str {
        match self {
            Action::Shell   => ">_", Action::Forward => "→ ", Action::Tunnel  => "⇄ ",
            Action::Fwd     => "← ", Action::Copy    => "⊕ ", Action::Sync    => "↺ ",
            Action::Ping    => "◎ ", Action::Proxy   => "⬡ ", Action::Scan    => "⊙ ",
            Action::Share   => "↑ ", Action::Watch   => "◉ ", Action::Punch   => "><",
            Action::Route   => "↪ ", Action::Mount   => "⊞ ",
        }
    }

    fn name(self) -> &'static str {
        match self {
            Action::Shell   => "Shell",   Action::Forward => "Forward",
            Action::Tunnel  => "Tunnel",  Action::Fwd     => "Reverse",
            Action::Copy    => "Copy",    Action::Sync    => "Sync",
            Action::Ping    => "Ping",    Action::Proxy   => "Proxy",
            Action::Scan    => "Scan",    Action::Share   => "Share",
            Action::Watch   => "Watch",   Action::Punch   => "Punch",
            Action::Route   => "Route",   Action::Mount   => "Mount",
        }
    }
    ...
    fn needs_param(self) -> bool { self.param_label().is_some() }
    ...
    fn to_args(self, host: &str, param: &str) -> Vec<String> {
        ...
        match self {
            ...
            Action::Scan  => {
                ...
                if !param.is_empty() { a.extend(["--ports".into(), param]); }
                a
            }
            Action::Share => {
                let mut a = vec!["share".into()];
                if !param.is_empty() { a.push(param); }
                a
            }
            Action::Watch => {
                if !param.is_empty() {
            ...
            Action::Mount => {
                let mp = if param.is_empty() { "/mnt/remote".into() } else { param };
...
fn load_recent() -> Vec<Recent> {
    ...
    for line in text.lines().rev() {
        if let Ok(v) = serde_json::from_str::<serde_json::Value>(line) {
            ...
            if remote.is_empty() || !remote.contains('@') { continue; }
...
fn format_ts_ago(ts: &str) -> String {
    if ts.len() >= 16 { format!("{} {}", &ts[5..10], &ts[11..16]) } else { ts.to_string() }
}
...
#[derive(Debug, Clone, Copy, PartialEq)]
enum Focus { Host, Actions, Param, Recent }
...
impl App {
    fn new() -> Self {
        ...
        if !recent.is_empty() { recent_state.select(Some(0)); }
        Self {
            focus: Focus::Host,
            host: String::new(),
            host_cursor: 0,
    ...
    fn action(&self) -> Action { Action::ALL[self.action_idx] }
    fn needs_param(&self)      -> bool { self.action().needs_param() }
    ...
    fn build_args(&self)    -> Vec<String> { self.action().to_args(&self.host, &self.param) }
    ...
    fn on_command_return(&mut self, cmd: String, exit_code: i32) {
        ...
        if !self.recent.is_empty() {
            self.recent_state.select(Some(if had { prev.min(self.recent.len() - 1) } else { 0 }));
        }
    }
    ...
    fn insert_char(&mut self, ch: char) {
        match self.focus {
            Focus::Host  => { self.host.insert(self.host_cursor, ch); self.host_cursor += ch.len_utf8(); }
            Focus::Param => { self.param.insert(self.param_cursor, ch); self.param_cursor += ch.len_utf8(); }
            _ => {}
        }
    }
    ...
    fn backspace(&mut self) {
        match self.focus {
            Focus::Host  if self.host_cursor  > 0 => { let p = prev_boundary(&self.host,  self.host_cursor);  self.host.remove(p);  self.host_cursor  = p; }
            Focus::Param if self.param_cursor > 0 => { let p = prev_boundary(&self.param, self.param_cursor); self.param.remove(p); self.param_cursor = p; }
            _ => {}
        }
    }
    ...
    fn kill_to_end(&mut self) {
        match self.focus {
            Focus::Host  => { self.host.truncate(self.host_cursor); }
            Focus::Param => { self.param.truncate(self.param_cursor); }
            _ => {}
        }
    }
    ...
    fn action_up(&mut self) {
        if self.action_idx > 0 { self.action_idx -= 1; self.param.clear(); self.param_cursor = 0; }
    }

    fn action_down(&mut self) {
        if self.action_idx + 1 < Action::ALL.len() {
            self.action_idx += 1; self.param.clear(); self.param_cursor = 0;
        }
    }

    fn recent_up(&mut self) {
        if let Some(i) = self.recent_state.selected() && i > 0 { self.recent_state.select(Some(i - 1)); }
    }

    fn recent_down(&mut self) {
        if let Some(i) = self.recent_state.selected()
            && i + 1 < self.recent.len() { self.recent_state.select(Some(i + 1)); }
    }
    ...
    fn set_action(&mut self, idx: usize) {
        ...
        self.param.clear();
        self.param_cursor = 0;
        self.focus = if Action::ALL[idx].needs_param() { Focus::Param } else { Focus::Actions };
    }

    fn tab_next(&mut self) {
        self.focus = match self.focus {
            Focus::Host    => Focus::Actions,
            Focus::Actions => if self.needs_param() { Focus::Param } else if !self.recent.is_empty() { Focus::Recent } else { Focus::Host },
            Focus::Param   => if !self.recent.is_empty() { Focus::Recent } else { Focus::Host },
    ...
    fn tab_prev(&mut self) {
        self.focus = match self.focus {
            Focus::Host    => if !self.recent.is_empty() { Focus::Recent } else if self.needs_param() { Focus::Param } else { Focus::Actions },
            Focus::Actions => Focus::Host,
            Focus::Param   => Focus::Actions,
            Focus::Recent  => if self.needs_param() { Focus::Param } else { Focus::Actions },
        };
    }
}
...
fn draw(f: &mut Frame, app: &mut App) {
    ...
    let title_r = Line::from(vec![
        Span::styled(format!(" id:{} ", app.identity_fp), style_dim()),
    ]).right_aligned();
    ...
    if !app.recent.is_empty() {
        ...
        let list = List::new(items)
            .block(Block::default()
                .title(Line::from(vec![Span::raw(" "), Span::styled("Recent", style_bold(C_ACCENT)), Span::raw(" ")]))
                .borders(Borders::ALL)
    ...
    // Host input
    {
        ...
        let w = render_input(&app.host, app.host_cursor, focused, "user@hostname")
            .block(Block::default()
                .title(Line::from(vec![Span::raw(" "), Span::styled("Host", style_bold(C_ACCENT)), Span::raw(" ")]))
                .borders(Borders::ALL)
                .border_style(border_style(focused)));
    ...
    // Action list
    {
        ...
        for (i, a) in Action::ALL.iter().enumerate() {
            ...
            let (key_style, icon_style, name_style, desc_style) = if selected && focused {
                (
                    Style::default().bg(C_HL_BG).fg(C_HL_FG).add_modifier(Modifier::BOLD),
        ...
        let display_sel = if app.action_idx >= 9 { app.action_idx + 1 } else { app.action_idx };
        let mut action_state = ListState::default();
        action_state.select(Some(display_sel));

        let shortcut_hint = Line::from(Span::styled(" [1-9·0·w·p·r·m] ", style_dim())).right_aligned();
        let list = List::new(items)
            .block(Block::default()
                .title_top(Line::from(vec![Span::raw(" "), Span::styled("Action", style_bold(C_ACCENT)), Span::raw(" ")]))
    ...
    if needs_param && rows[3].height > 0 {
        ...
        let w = render_input(&app.param, app.param_cursor, focused, placeholder)
            .block(Block::default()
                .title(Line::from(vec![Span::raw(" "), Span::styled(label, style_bold(C_ACCENT)), Span::raw(" ")]))
                .borders(Borders::ALL)
                .border_style(border_style(focused)));
    ...
    if let Some((ref cmd, code)) = app.last_run
        && rows[5].height > 0
    {
        let (icon, col) = if code == 0 { ("✓ ", C_GREEN) } else { ("✗ ", C_RED) };
        let suffix = if code == 0 { String::new() } else { format!("  exit {code}") };
        f.render_widget(Paragraph::new(Line::from(vec![
            Span::styled(icon, style_bold(col)),
            Span::styled(cmd.clone(), Style::default().fg(col).add_modifier(Modifier::BOLD)),
    ...
    if rows[6].height > 0 {
        let line = if let Some(ref msg) = app.validation {
            Line::from(Span::styled(format!("  ⚠  {msg}"), Style::default().fg(C_YELLOW)))
        } else if app.ready() {
            Line::from(vec![
            ...
            Line::from(Span::styled("  enter a host above  (user@hostname)", style_dim()))
        } else if app.needs_param() && app.param.trim().is_empty() {
            Line::from(Span::styled(
                format!("  enter {} above  (e.g. {})", app.action().param_label().unwrap_or("param"), app.action().param_placeholder()),
                style_dim(),
            ))
        } else {
            Line::from(Span::raw(""))
    ...
    if app.show_help {
        ...
        let help_text = vec![
            Line::from(vec![Span::styled(" Keyboard shortcuts", style_bold(C_ACCENT))]),
            Line::from(""),
            Line::from(vec![Span::styled(" Navigation", style_bold(C_WHITE))]),
            Line::from(vec![Span::styled("  Tab / Shift-Tab  ", style_accent()), Span::styled("cycle focus panels", style_muted())]),
            Line::from(vec![Span::styled("  ↑ ↓  /  j k     ", style_accent()), Span::styled("move in list", style_muted())]),
            Line::from(vec![Span::styled("  ← →             ", style_accent()), Span::styled("move text cursor", style_muted())]),
            Line::from(""),
            Line::from(vec![Span::styled(" Action shortcuts", style_bold(C_WHITE))]),
            Line::from(vec![Span::styled("  1-9             ", style_accent()), Span::styled("Shell → Scan", style_muted())]),
            Line::from(vec![Span::styled("  0               ", style_accent()), Span::styled("Share", style_muted())]),
            Line::from(vec![Span::styled("  w p r m         ", style_accent()), Span::styled("Watch  Punch  Route  Mount", style_muted())]),
            Line::from(""),
            Line::from(vec![Span::styled(" Text editing", style_bold(C_WHITE))]),
            Line::from(vec![Span::styled("  Ctrl-A / Home   ", style_accent()), Span::styled("start of line", style_muted())]),
            Line::from(vec![Span::styled("  Ctrl-E / End    ", style_accent()), Span::styled("end of line", style_muted())]),
            Line::from(vec![Span::styled("  Ctrl-K          ", style_accent()), Span::styled("delete to end of line", style_muted())]),
            Line::from(vec![Span::styled("  Ctrl-W          ", style_accent()), Span::styled("delete word back", style_muted())]),
            Line::from(""),
            Line::from(vec![Span::styled(" General", style_bold(C_WHITE))]),
            Line::from(vec![Span::styled("  Enter           ", style_accent()), Span::styled("run command / select recent", style_muted())]),
            Line::from(vec![Span::styled("  ?               ", style_accent()), Span::styled("toggle this help", style_muted())]),
            Line::from(vec![Span::styled("  Esc / q         ", style_accent()), Span::styled("quit (outside text fields)", style_muted())]),
            Line::from(vec![Span::styled("  Ctrl-C          ", style_accent()), Span::styled("quit always", style_muted())]),
        ];

        let block = Block::default()
            .title(Line::from(vec![Span::raw(" "), Span::styled("Help", style_bold(C_ACCENT)), Span::raw("  "), Span::styled("press ? to close", style_dim()), Span::raw(" ")]))
            .borders(Borders::ALL)
...
pub fn run() -> Result<()> {
    ...
    loop {
        ...
        if key.modifiers.contains(KeyModifiers::CONTROL) {
            match key.code {
                KeyCode::Char('w') if in_text => { app.delete_word(); continue; }
                KeyCode::Char('a') if in_text => { app.cursor_home(); continue; }
                KeyCode::Char('e') if in_text => { app.cursor_end(); continue; }
                KeyCode::Char('k') if in_text => { app.kill_to_end(); continue; }
                _ => {}
            }
        }
        ...
        match key.code {
            ...
            KeyCode::Char('?') if !in_text => { app.show_help = true; continue; }

            KeyCode::Tab => {
                if key.modifiers.contains(KeyModifiers::SHIFT) { app.tab_prev(); } else { app.tab_next(); }
            }
            ...
            KeyCode::Char(c) if !in_text => {
                ...
                match c {
                    'j' => match app.focus { Focus::Actions => app.action_down(), Focus::Recent => app.recent_down(), _ => {} },
                    'k' => match app.focus { Focus::Actions => app.action_up(),   Focus::Recent => app.recent_up(),   _ => {} },
                    _ => {}
                }
            }
            ...
            KeyCode::Backspace => { app.backspace(); app.validation = None; }
            KeyCode::Delete    => { app.cursor_right(); app.backspace(); }
                ...
                _ => {
                    if app.ready() {
                        ...
                        app.validation = Some(format!(
                            "enter {label}  (e.g. {})", app.action().param_placeholder()
                        ));
            ...
            KeyCode::Char(c) if in_text => { app.validation = None; app.insert_char(c); }
